...
const AdminOption = () => {
  ...
  React.useEffect(() => {
    ...
    axios.get(`http://192.168.0.240:5000/api/paper/all`)
      .then((response) => {
  ...
  const handleRemoveAll = async () => {
    try {
      const response = await axios.post(`http://192.168.0.240:5000/api/quiz/removeAll/${paperId}`); 
  ...
  const handleResetScore = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://192.168.0.240:5000/api/user/resetScore');
  ...
  const handleDownloadReports = async () => {
    try {
      const response = await axios.get(
        `http://192.168.0.240:5000/api/excelInsert/downloadZip/${paperId}`, // Updated route to include paperId
        { responseType: 'blob' }
      );
